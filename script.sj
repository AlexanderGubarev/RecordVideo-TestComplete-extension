...
/*
	Text constants
*/
var msg_already_running = 
"The video is already in recording state. Please see Additional information"
var msg_already_running_additional = 
"You need to stop previous video recording before starting the new \
one. \r\nIf you see "+"%s"+".exe process, please close it manually."

var msg_start = 
"The video recording is started. You can find \
recorded video in Logs folder."
var msg_start_additional = 
"The quality is: "+"%s"+". \r\nYou can change the quility of videos by redefining \
VideoQuality parameter.\r\n\r\n"+"The video file will be created:\r\n"+"%s";

var msg_not_installed = 
"Unable to record video. Please check that VLC video player is installed."
var msg_not_installed_additional = 
"<p>You can download necessary VLC video player here:<br/>"+'<a href="'+"%s"+'" \
target="_blank">'+"%s"+'</a></p>'

var msg_not_started = 
"The video recording was not even started!"
var msg_not_started_additional = 
"Unable to detect working instance of VLC application. \
Please, check that you start recording in your test."

var msg_existing = 
"The video recording was not even started! Please see Additional information"
var msg_existing_additional = 
"It seems the previos instance of VLC player was not closed. \
\r\nIf you see "+"%s"+".exe process, please close it manually."

var msg_somethingwrong =
"Something was wrong during the recording process. Please see Additional Information"
var msg_somethingwrong_additional =
"Please try to launch the player manually with command line:\r\n\r\n"+"\""+"%s"+"\""+" "+"%s"

var msg_stop = 
"The video recording is stopped."
var msg_stop_additional = 
"The video file has been created:\r\n"+"%s"
...
function Start(VideoQuality)
{
  ...
  if (isVLCExists)
  {
    Log.Warning(msg_already_running, aqString.Format(msg_already_running_additional, 
                proc_name))
    return
  }
  ...
  if (isVLCInstalled())
  {
    ...
    isStarted = true;
    LaunchRecording()
    Log.Message(msg_start, aqString.Format(msg_start_additional, Quality, video_file_path));
  }
  else
  {
    var pmHigher = 300
    ...
    Log.Warning(msg_not_installed, aqString.Format(msg_not_installed_additional, website, 
                website), pmHigher, attr)
  }
...
function Stop()
{
  ...
  if (!isVLCExists)
    Log.Warning(msg_not_started, msg_not_started_additional)
  else if (!isStarted)
  {
	 Log.Warning(msg_existing, aqString.Format(msg_existing_additional, proc_name))
  }
  else
  {
    StopRecording()
  ...
  for (var i=0;i<20;i++)
    ...
    if (aqFile.Exists(aqString.Unquote(video_file_path)))
      Log.Link(video_file_path, msg_stop, aqString.Format(msg_stop_additional, 
              video_file_path))
    else
      Log.Warning(msg_somethingwrong, aqString.Format(msg_somethingwrong_additional, 
                  proc_name, getVLCPath(), prepareStartParamsString()))
  }
  return video_file_path
}
